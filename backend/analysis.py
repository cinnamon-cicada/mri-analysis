...
# Benchmark stats loaded once at import time; generated by benchmark/generate_stats.py
_BENCHMARK_PATH = Path(__file__).parent / "benchmark" / "adhd_stats.json"
_benchmark: dict | None = None
...
def compare_to_benchmark(subject_dir: str) -> dict:
    """
    Compare a single subject's FastSurfer output against the precomputed
    ADHD benchmark statistics.
    ...
    dict
        {
          "volume_percentiles": [[region, percentile], ...],
          "thickness_percentiles": [[region, percentile], ...],
        }
    ...
    def percentile(value: float, mean: float, std: float) -> float:
        z = (value - mean) / std if std != 0 else 0
        return 0.5 * (1 + erf(z / sqrt(2)))

    volume_percentiles = []
    for region, stats in benchmark.get("volumes", {}).items():
        if region not in volumes.get("volumes", {}):
            volume_percentiles.append([region, None])
            continue
        p = percentile(volumes["volumes"][region], stats["mean"], stats["std"])
        volume_percentiles.append([region, p])

    thickness_percentiles = []
    for region, stats in benchmark.get("thickness", {}).items():
        if region not in thickness.get("thickness", {}):
            thickness_percentiles.append([region, None])
            continue
        p = percentile(thickness["thickness"][region], stats["mean"], stats["std"])
        thickness_percentiles.append([region, p])
